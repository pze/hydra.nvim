...
local nvim_buf_get_option = function(buf, opt)
   if not buf or buf == 0 then
      buf = api.nvim_get_current_buf()
   end
   return api.nvim_get_option_value(opt, { buf = buf })
end
...
function ma:initialize(augroup)
   ...
   ---@param opt string
      function(opt)
         return nvim_buf_get_option(0, opt)
      end,
...
function ma:_set_buf_option(opt, val)
   ...
   if not self.original.bo[bufnr][opt] then
      self.original.bo[bufnr][opt] = nvim_buf_get_option(bufnr, opt)
   end
   api.nvim_set_option_value(opt, val, {
      buf = bufnr,
   })
